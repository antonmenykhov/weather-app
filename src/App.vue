<template>
  <div id="app">
    <div v-if="auth" class="container grid">
      <Menu :menu="menu[role]" :user="name"></Menu>
      <div class="page">
        <h3 class="page-title">
          {{ templateTitle[currentTemplate] }} <span>{{ city }}</span>
        </h3>
...
export default {
  ...
  computed: {
    name() {
      return this.auth.idTokenParsed.name;
    },
    role() {
      return this.auth.resourceAccess["app-vue"].roles[0];
    },
    auth() {
  ...
  created() {
    ...
    this.$eventBus.$on("showDays", (data) =>
      this.showDays(data.template, data.item ? data.item : null)
    );
  ...
  methods: {
    ...
    changeCity(city) {
      axios
        .get(api.generateGeoResponse(city))
        .then((response) => {
          if (response.data.length > 0) {
            this.noItems = false;
            this.city = response.data[0].local_names.ru;
            axios
              .get(api.generateWeeklyResponse(response.data[0].lat, response.data[0].lon))
              .then((response2) => {
              ...
              .catch(function (error) {
                ...
                this.$eventBus.$emit(
                  "showError",
                  "Ошибка при выполнении запроса, попробуйте еще раз"
                );
              });
          } else {
            this.noItems = true;
